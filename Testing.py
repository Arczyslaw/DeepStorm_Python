...
import argparse
from ModelCNN import buildModel, project_01, normalize_im
import h5py
import time
...
def test_model(datafile, weights_file, meanstd_file, savename, \
               upsampling_factor=8, debug=0, results='test_results.hdf5'):
    """
    ...
    # load mean and std
    f = h5py.File(meanstd_file, 'r')
    test_mean = np.array(f['mean_test'][()])
    test_std = np.array(f['std_test'][()]) 
    ...
    grey = plt.cm.gray
    ax1.imshow(WideField, cmap = grey)
    ax1.set_title('Obraz oryginalny')
    ax2.imshow(Recovery, cmap = grey)
    ax2.set_title('Suma predykcji')
    f.subplots_adjust(hspace=0)
    plt.show()
    
    # Save predictions to a matfile to open later in matlab
    im  =Image.fromarray(Recovery, mode='F')
    im.save(savename, "TIFF")
    ...
    if debug:
        with h5py.File(results, 'w') as prediction:
            prediction.create_dataset('prediction', predicted_density.shape, dtype = 'f', data=predicted_density)
            prediction.close()
    return f
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results', type=str, default = 'test_results.hdf5', help='file with prediction')
    ...
    test_model(abspath(args.datafile), abspath(args.weights_name), \
               ...
               args.upsampling_factor, args.debug, args.results)
