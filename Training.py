...
from ModelCNN import LossHistory, buildModel, project_01, normalize_im
    
# define a function that trains a model for a given data SNR and density
def train_model(filename, weights_name, meanstd_name, epoki = 50):
    ...
    patches = matfile['patches'][()]
    heatmaps = 100.0 * matfile['heatmaps'][()]
    ...
    train_history = model.fit_generator(datagen.flow(X_train_norm, Y_train, batch_size=16), \
                                        steps_per_epoch=500, epochs=epoki, verbose=1, \
    ...
    plt.plot(loss)
    plt.plot(val_loss)
    plt.legend(['Na zbiorze treningowym', 'Na zbiorze walidacyjnym'])
    plt.xlabel('Epoka')
    plt.ylabel('Funkcja Straty')
    plt.title("Wartości funkcji straty podczas trenowania modelu")
    plt.show()
    
    # Save datasets to a matfile to open later in matlab
    
    with h5py.File(meanstd_name, 'w') as mean:
        mean.create_dataset('mean_test', data = np.array(mean_val_test))
        mean.create_dataset('std_test',  data = np.array(std_val_test))
        mean.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', type=int, default = 50, help = "number epochs per training" )
    ...
    train_model(abspath(args.filename), abspath(args.weights_name), abspath(args.meanstd_name), args.epochs)
